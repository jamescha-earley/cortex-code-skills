...
# Strings that appear in the input bar when cortex is ready for input.
# "Type your message" is the default; "auto-accept" appears when --auto-accept-plans is on.
INPUT_BAR_MARKERS = ["Type your message", "auto-accept"]


def _input_bar_visible(content):
    """Return True if the cortex input bar is visible (ready for user input)."""
    return any(marker in content for marker in INPUT_BAR_MARKERS)
...
def _detect_interactive_prompt(content):
    ...
    IMPORTANT: Only checks the last ~10 non-empty lines to avoid false positives
    from numbered lists in regular output (e.g., plan steps, bullet points).
    """
    lines = [l.strip() for l in content.strip().splitlines() if l.strip()]
    ...
    if "❯" in tail_text or "›" in tail_text:
        if "○" in tail_text or "●" in tail_text or "◯" in tail_text:
            return "radio"

    # Checkbox: square markers
    if any(marker in tail_text for marker in ["◻", "◼", "☐", "☑", "[ ]", "[x]", "◯", "●"]):
        if "❯" in tail_text or "›" in tail_text:
            return "checkbox"

    # Numbered options: only match short numbered lines in the tail that look
    # like menu choices (e.g., "1. Yes", "2. No", "3. Record MP4"),
    # NOT long paragraphs or plan steps that happen to start with a number.
    numbered_pattern = re.compile(r'^\d+\.\s+\S.{0,60}$')
    numbered_lines = [l for l in tail if numbered_pattern.match(l)]
    if len(numbered_lines) >= 2:
        nums = sorted(set(int(l.split(".")[0]) for l in numbered_lines if l[0].isdigit()))
...
def wait_for_response(timeout=RESPONSE_TIMEOUT):
    ...
    while timeout == 0 or (time.time() - start < timeout):
        ...
        # Phase 1: Wait for the input prompt to disappear (or detect that
        # Cortex started processing). With --auto-accept-plans the input bar
        # text ("Plans will be auto accepted...") persists even during processing,
        # so we also check for content changes as a sign the prompt was accepted.
        if not prompt_disappeared:
            input_gone = not _input_bar_visible(content)
            content_changed = (last_content and content != last_content)
            processing = "esc to interrupt" in content

            if input_gone or content_changed or processing:
        ...
        # Done: input prompt reappears (or auto-accept banner is showing)
        # BUT only if "esc to interrupt" is NOT present -- that means Cortex
        # is still actively processing even though the input bar text is visible.
        if _input_bar_visible(content) and "esc to interrupt" not in content:
